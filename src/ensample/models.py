"""Pydantic models that mirror the Ensample specification."""
...
class LLMConfig(BaseModel):
    ...
    api_key_env_var: str | None = Field(default=None, description="Environment variable name for the API key. If not provided, will look for a default for the provider.")
    base_url: str | None = Field(default=None, description="Custom base URL for the LLM API endpoint.")
    temperature: float = Field(default=0.1, ge=0.0, le=2.0, description="Sampling temperature for text generation.")
    top_p: float = Field(default=1.0, ge=0.0, le=1.0, description="Cumulative probability for nucleus sampling.")
    max_tokens: int | None = Field(default=None, ge=0, description="Maximum number of tokens to generate.")
    stop_sequences: list[str] | None = Field(default=None, description="Sequences at which to stop generation.")
    # Add any other provider-specific parameters as a generic dict
    extra_params: dict[str, Any] | None = Field(default=None, description="Provider-specific parameters not covered by standard fields.")
...
class RedFlagRule(BaseModel):
    ...
    value: str | None = Field(default=None, description="Regex pattern, keyword, or length threshold (as string 'N_TOKENS').")
...
class RedFlagConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="Whether red-flagging is enabled.")
...
class MDAPInput(BaseModel):
    ...
    ensemble_config: EnsembleConfig | None = Field(default=None, description="Optional: overrides default ensemble configuration.")
    voting_k: int = Field(default=3, ge=0, description="The 'k' value for 'first-to-ahead-by-k' voting. k=0 means greedy/no voting beyond first valid response, k=1 means simple majority.")
    red_flag_config: RedFlagConfig | None = Field(default=None, description="Optional: overrides default red-flagging configuration.")
    output_parser_schema: dict[str, Any] | None = Field(default=None, description="JSON schema or other structured definition for parsing LLM output to extract a canonical response. If absent, output is treated as plain text.")
    fast_path_enabled: bool = Field(default=False, description="If true, short-circuits voting if a 'sufficiently confident' response is found early (e.g., first valid response if k=0).")
    # Contextual fields for logging/tracing specific to the client's workflow
    client_request_id: str | None = Field(default=None, description="Client-defined ID for the overall request/task this MDAP execution is part of.")
    client_sub_step_id: str | None = Field(default=None, description="Client-defined ID for a specific sub-step within the request/task.")
...
class MDAPMetrics(BaseModel):
    ...
    estimated_llm_cost_usd: float = Field(default=0.0, ge=0.0, description="Estimated total LLM API cost for this MDAP execution.")
...
class MDAPOutput(BaseModel):
    ...
    error_message: str | None = Field(default=None, description="Error message if MDAP execution failed or timed out.")


# --- Additional utility models ---
class TokenUsage(BaseModel):
    """Detailed token usage information."""
    prompt_tokens: int | None = Field(default=None, ge=0, description="Number of tokens used for the prompt.")
    completion_tokens: int | None = Field(default=None, ge=0, description="Number of tokens used for the completion.")
    total_tokens: int | None = Field(default=None, ge=0, description="Total number of tokens used.")
    estimated: bool = Field(default=False, description="Whether these token counts are estimated rather than actual.")
...
class LLMResponse(BaseModel):
    ...
    latency_ms: int = 0
    tokens_used: TokenUsage | None = Field(default=None, description="Detailed token usage information.")
